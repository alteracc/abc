...
#    obs_1=pd.read_csv('19Jancluster.csv',usecols=[3],header=None,skiprows=[0])
    ...
    new_obs['cluster']=['c'+str(i) for i in labels]
    print(new_obs)
    return new_obs
...
model.fit(train[3].values.reshape(len(train),1), train[['cx','cy']].values.reshape(len(train),2))

## for predicting
predictions = model.predict(test[3].values.reshape(len(test),1))
print(mean_squared_error(predictions, test[['cx','cy']].values.reshape(len(test),2)))

## accuracy
accuracy = model.score(test[3].values.reshape(len(test),1),test[['cx','cy']].values.reshape(len(test),2))
print(abs(accuracy)*100,'%')

print(test[3].values)
print(test[['cx','cy']].values)
print(predictions)


# Using Predictions data to test for Cluster in logistic regression 
#model2= LogisticRegression()
#model2.fit(train[['cx','cy']].values.reshape(len(train),2), train['cluster'].values.reshape(len(train),1))
#pred_data=pd.DataFrame()
#pred_data['pred_x']=[predictions[j][0] for j in range(len(predictions))]
#pred_data['pred_y']=[predictions[j][1] for j in range(len(predictions))]
